...
int main(int argc, char *argv[]){
  ...
  if( run_test("set_temp_from_ports() 0 F") ) {
    PRINT_TEST;
    // Tests whether sensor value of 0 with all 0 status
    // port is handled correctly.
    THERMO_SENSOR_PORT  = 0;
  ...
  if( run_test( "set_temp_from_ports() 128 C/F" ) ) {
    PRINT_TEST;
  ...
  if( run_test( "set_temp_from_ports() freezing C" ) ) {
    PRINT_TEST;
  ...
  if( run_test( "set_temp_from_ports() freezing F" ) ) {
    PRINT_TEST;
  ...
  if( run_test( "set_temp_from_ports() rounding C" ) ) {
    PRINT_TEST;
    THERMO_STATUS_PORT  = 0b00000000; // bit 5 is 0, Celsius
    THERMO_DISPLAY_PORT = -1;
  ...
  if( run_test( "set_temp_from_ports() status nonzero" ) ) {
    PRINT_TEST;
  ...
  if( run_test( "set_temp_from_ports() sensor range" ) ) {
    PRINT_TEST;
  ...
  if( run_test( "set_temp_from_ports() status error" ) ) {
    PRINT_TEST;
  ...
  if( run_test( "set_temp_from_ports() wide range" ) ) {
    PRINT_TEST;
  ...
  if( run_test( "set_display_from_temp() extreme values" ) ) {
    PRINT_TEST;
  ...
  if( run_test( "set_temp() + set_display() 1" ) ) {
    PRINT_TEST;
    // Calls set_temp() and set_display() in sequence
    THERMO_SENSOR_PORT  = (234+450)*32 + 13;
    THERMO_STATUS_PORT  = 0b00000000; // celsius
